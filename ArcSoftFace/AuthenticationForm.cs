...
namespace ArcSoftFace
{
    public partial class AuthenticationForm : Form
    {
        ...
        private void buttonRecordNew_Click(object sender, EventArgs e)
        {
            if (comboBoxClass.SelectedIndex != 0) {
                //没有选课程，新建考勤记录失效
                FormRecord formRecord = new FormRecord(classes[comboBoxClass.SelectedIndex].Cname, classes[comboBoxClass.SelectedIndex].Cno);
                ...
                //更新
                int boxClassSelectedIndex = comboBoxClass.SelectedIndex;
                int stuClassSelectedIndex = comboBoxStudent.SelectedIndex;
                int recClassSelectedIndex = comboBoxRecord.SelectedIndex;
                importClass();
                comboBoxClass.SelectedIndex = boxClassSelectedIndex;
                comboBoxStudent.SelectedIndex = stuClassSelectedIndex;
        ...
        private void buttonRecordDelete_Click(object sender, EventArgs e)
        {
            if (comboBoxClass.SelectedIndex != 0 && records.Count > 0)
            {
                try
                {
                    var lines = File.ReadAllLines("..\\..\\..\\Database\\Class\\List\\" + classes[comboBoxClass.SelectedIndex].Cno + ".csv");
                    var strArray = lines[0].Split(',');
                    int col = 0;
                    string record = records[comboBoxRecord.SelectedIndex].Rname;
                    for (col = 0; col < strArray.Length; col++)
                    {
                        if (strArray[col] == record)
                            break;
                    }
                    if (col < strArray.Length)
                    {
                        ...
                        var claLines = File.ReadAllLines("..\\..\\..\\Database\\Class\\Class.csv");
                        string classCno = classes[comboBoxClass.SelectedIndex].Cno;
                        for (int i = 0; i < claLines.Length; i++) {

                            if (claLines[i].Split(',')[0] == classCno) {
                                strArray = claLines[i].Split(',');
                                strArray[2]=(int.Parse(strArray[2])-1).ToString();
                                claLines[i] = string.Join(",", strArray);
                                break;
                            }  
                        }
                        File.WriteAllLines("..\\..\\..\\Database\\Class\\Class.csv", claLines);
                        //更新
                        //恢复下标选择
                        int boxClassSelectedIndex = comboBoxClass.SelectedIndex;
                        int stuClassSelectedIndex = comboBoxStudent.SelectedIndex;
        ...
        private void buttonRecordDisplay_Click(object sender, EventArgs e)
        {
            if (comboBoxClass.SelectedIndex != 0&&records.Count!=0) {
                ...
                formDisplay.setLabel(classes[comboBoxClass.SelectedIndex].Cname, classes[comboBoxClass.SelectedIndex].Cno, records[comboBoxRecord.SelectedIndex].Rname);
